...
#    where("masters_program = ? and graduation_year = ?",program, year.to_s).order("human_name ASC")
  ...
  def emailed_recently(email_type)
    case email_type
      when :effort_log
        return self.effort_log_warning_email.nil? || (self.effort_log_warning_email < 1.day.ago) ? true : false
      when :sponsored_project_effort
        return self.sponsored_project_effort_last_emailed.nil? || (self.sponsored_project_effort_last_emailed < 1.day.ago) ? true : false
    end
  end
