class EffortLog < ActiveRecord::Base
  has_many :effort_log_line_items
  belongs_to :person
  ...
  def editable_by(current_user)
    ...
    if (current_user && current_user.id == person_id)
      ...
      if (Date.today >= Date.commercial(self.year, self.week_number, 1) && Date.today <= (Date.commercial(self.year, self.week_number, 7) + 7.day))
        return true
      end
    end
    return false
  ...
  def self.log_effort_week?(year, week_number)
    if AcademicCalendar.week_during_semester?(year, week_number)
      return true
    else
      return AcademicCalendar.spring_break(year).include?(week_number)
    end
  end
